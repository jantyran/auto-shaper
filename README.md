...
- **`/api/suggest`・`/api/extract` は認証不要**です(ステートレスなLLM中継のみで、保存処理を
  伴わないため意図的に未認証にしています)。サーバーに到達できる人は誰でも、自分の
  LLM APIキーを使ってこのエンドポイント経由でプロバイダを呼び出せます。信頼できない
  相手にサーバーを開放しない、または前段でIP制限・認証を挟んでください。
- **ログイン(`/api/auth/signup`・`/api/auth/login`)にレート制限がありません**。公開する場合は
  リバースプロキシ(nginx等)や `express-rate-limit` などでブルートフォース対策を追加することを
  推奨します。
