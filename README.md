...
2. **インポート先選択** — プリセット(Salesforce Lead / HubSpot Contact / Marketo Engage
   人物リスト など)、または独自のインポート用シート(ヘッダー行)をアップロード
   - プリセットの項目キーは各サービスの取り込み時の列名（Salesforce の API 参照名、
     HubSpot の内部プロパティ名、Marketo の REST API 名）に合わせてあるので、
     出力した CSV をそのまま読み込ませられます
   - 表示するカテゴリは「設定」で切り替えられます（既定は CRM と MA・メール配信）
