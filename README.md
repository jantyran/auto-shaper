...
初回アクセス時は、まず表示言語を English / 日本語 から選びます。言語は設定画面からいつでも切り替えられ、選択はこのブラウザに保存されます。その後、導入画面からサンプルデータを使ったガイドツアーを始められます。
...
npm run lint          # ESLint
npm run test          # 変換・推論・検証・国際化・ガイドツアーのテスト(vitest)
npm run build         # 本番ビルド
